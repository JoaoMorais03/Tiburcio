...
export const getImpactAnalysisTool = tool({
  ...
  inputSchema: z.object({
    target: z.string().describe("File path, function name, class name, or table name to analyze"),
    targetType: z
      .enum(["file", "function", "class", "table"])
    ...
    depth: z
      ...
      .max(3)
      .default(2)
      .describe("Traversal depth: 1=direct only, 2=two hops, 3=three hops"),
    repo: z
      .string()
      .optional()
      .describe("Filter to a specific repo name. Omit to search all repos."),
  }),
